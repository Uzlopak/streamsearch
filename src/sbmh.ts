export type MatchHandler = (isMatch: boolean, data?: Uint8Array) => void;
...
export default class SBMH {
  public maxMatches = Infinity;
  public matches = 0;

  private _occ = new Uint8Array(256);
  private _lookbehind_size = 0;
  private _bufpos = 0;
  private _lookbehind: Uint8Array;

  constructor(
    private readonly _needle: Uint8Array,
    private readonly _onInfo: MatchHandler
  ) {
  ...
  private _sbmh_feed(data: Uint8Array) {
    ...
    while (pos <= len - needle_len) {
      ...
      if (
        ...
        ++this.matches;
        if (pos > 0) this._onInfo(true, data.subarray(this._bufpos, pos));
  ...
  private _sbmh_lookup_char(data: Uint8Array, pos: number) {
    if (pos < 0) return this._lookbehind[this._lookbehind_size + pos];
    else return data[pos];
  }

  private _sbmh_memcmp(data: Uint8Array, pos: number, len: number) {
    let i = 0;

    while (i < len) {
      if (this._sbmh_lookup_char(data, pos + i) === this._needle[i]) ++i;
      else return false;
    }
    return true;
  }
